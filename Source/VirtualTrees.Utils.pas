...
uses
  DelphiCompat, Types, LCLIntf, LCLType, Graphics, LclExt, SysUtils, ExtCtrls,
...
procedure CreateRegion(const AIm: TImage; const AMaskColor: TColor; out ARegion: HRGN);
  ...
  begin
    ...
    repeat
      if (X2 <= AIm.Picture.Bitmap.Width - 1) and (Y <= AIm.Picture.Bitmap.Height - 1) then
        Color := AIm.Picture.Bitmap.Canvas.Pixels[X2, Y]
      else
        Color := -1;
      if (Color <> AMaskColor) then
      begin
  	if (X1 <> X2) then
  	begin
